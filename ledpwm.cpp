...
void setup_ledpwm() {
  ...
  // set LED pin modes for all of PORTB. We can do this here because we don't use these anywhere else
  DigitalPin<MODE_LED_PIN_1> mode_pin_1;
  DigitalPin<MODE_LED_PIN_2> mode_pin_2;
  DigitalPin<MODE_LED_PIN_3> mode_pin_3;
  DigitalPin<MODE_LED_PIN_4> mode_pin_4;

  mode_pin_1.config(OUTPUT, LOW);
  mode_pin_2.config(OUTPUT, LOW);
  mode_pin_3.config(OUTPUT, LOW);
  mode_pin_4.config(OUTPUT, LOW);
